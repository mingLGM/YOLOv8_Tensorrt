...
static std::vector<uint8_t> load_file(const string &file) {
  // 反序列化
  _buffer_ buffer;
  if (is_endwith(file.c_str(), ".hctrt"))
  {
      buffer.read_from_file(file.c_str());
      buffer = HcoxDecrypt(buffer).get_decrypted_buffer_by_key("model");
      if (buffer.isnull())
      {
          glogger.log(HCLog::Severity::kERROR, "buffer.isnull");
          return {};
      }
  }else { buffer.read_from_file(file.c_str()); }
  std::vector<uint8_t> data(buffer.get_buffer_size());
  std::copy(buffer(), buffer() + buffer.get_buffer_size(), data.begin());
  return data;
}
...
class InferImpl : public Infer {
 public:
  ...
  size_t getDataType(nvinfer1::DataType type)
  {
      switch (type)
      {
  ...
  virtual size_t get_head_byteSize(int index) override {
      int numel_ = numel(index);
      nvinfer1::DataType dataType = this->context_->engine_->getBindingDataType(index);
      size_t byteSize = numel_ * getDataType(dataType);
      return byteSize;
  }
